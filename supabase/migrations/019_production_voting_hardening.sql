...
BEGIN
  ...
  INSERT INTO public.voting_settings (
    id, is_voting_open, show_live_counts, results_finalized, scheduled_start, scheduled_end, votes_per_category, updated_at
  )
  VALUES (1, true, true, false, NULL, NULL, 5, NOW())
  ON CONFLICT (id) DO UPDATE SET
    is_voting_open = true,
  ...
  RETURN jsonb_build_object(
    'success', true,
    'message', 'All votes, submissions, sessions, appreciation notes, and student profiles have been completely reset.'
  );
